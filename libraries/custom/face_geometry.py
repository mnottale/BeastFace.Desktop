...
print('***')
print(np.mean(canonical_metric_landmarks[2,:]))
with open('/tmp/canonical.xyz', 'w') as f:
    for i in range(canonical_metric_landmarks.shape[1]):
        f.write('{} {} {}\n'.format(canonical_metric_landmarks[0,i],canonical_metric_landmarks[1,i],canonical_metric_landmarks[2,i]))
sz = 4096
f = np.zeros((sz,sz), dtype=np.uint8)
f[:,:] = 255
minx = np.min(canonical_metric_landmarks[0,:])
maxx = np.max(canonical_metric_landmarks[0,:])
miny = np.min(canonical_metric_landmarks[1,:])
maxy = np.max(canonical_metric_landmarks[1,:])
hextendx = max(-minx, maxx)
hextendy = max(-miny, maxy)
hextend = max(hextendx, hextendy)
hextend *= 1.3
import cv2
for i in range(canonical_metric_landmarks.shape[1]):
    ptx = canonical_metric_landmarks[0,i]
    pty = canonical_metric_landmarks[1,i]
    x = -ptx / hextend * (sz//2) + sz // 2
    y = -pty / hextend * (sz//2) + sz // 2
    f[round(y),round(x)] = 0
    cv2.putText(f, str(i), (round(x), round(y)), cv2.FONT_HERSHEY_SIMPLEX, 0.3, 0, 1)
cv2.imwrite('/tmp/canonical13-index.png', f)
#with open('/tmp/canonicalscale.txt', 'w') as sf:
#    sf.write(str(hextend))
